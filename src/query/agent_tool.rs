...
use crate::api::{AnthropicClient, ModelRegistry, ProviderRegistry};
use crate::core::types::Message;
use crate::tools::{PermissionLevel, Tool, ToolContext, ToolResult};
...
impl Tool for AgentTool {
    fn name(&self) -> &str {
        crate::core::constants::TOOL_NAME_AGENT
    }
    ...
    async fn execute(&self, input: Value, ctx: &ToolContext) -> ToolResult {
        ...
        let agent_tools: Vec<Box<dyn Tool>> = if let Some(ref allowed) = params.tools {
            ...
            all.into_iter()
                .filter(|t| t.name() != crate::core::constants::TOOL_NAME_AGENT)
                .collect()
        };
        ...
        if params.run_in_background {
            let mut task = crate::core::tasks::BackgroundTask::new(format!(
                "subagent: {}",
                params.description
            ));
            task.id = agent_id.clone();
            let _ = crate::core::tasks::global_registry().register(task);

            // Re-create the tool list inside the closure so it is owned and Send.
            let agent_tools_bg: Vec<Box<dyn Tool>> = crate::tools::all_tools()
                .into_iter()
                .filter(|t| t.name() != crate::core::constants::TOOL_NAME_AGENT)
                .collect();
            ...
            tokio::spawn(async move {
                ...
                let cancelled = matches!(
                    crate::core::tasks::global_registry()
                        .get(&agent_id_bg)
                        .map(|t| t.status),
                    Some(crate::core::tasks::TaskStatus::Cancelled)
                );

                let result_text = format_outcome(outcome);
                crate::core::tasks::global_registry().append_output(&agent_id_bg, &result_text);

                if !cancelled {
                    let status = if result_text.starts_with("[Agent error:")
                        || result_text.starts_with("[Agent stopped:")
                    {
                        crate::core::tasks::TaskStatus::Failed(result_text.clone())
                    } else {
                        crate::core::tasks::TaskStatus::Completed
                    };
                    crate::core::tasks::global_registry().update_status(&agent_id_bg, status);
                }
...
pub fn init_team_swarm_runner() {
    let runner: crate::tools::AgentRunFn = Arc::new(
        |description: String,
         prompt: String,
         tools: Option<Vec<String>>,
         ...
         ctx: Arc<crate::tools::ToolContext>| {
            ...
            Box::pin(async move {
                let anthropic_key = ctx.config.resolve_anthropic_api_key().unwrap_or_default();
                let anthropic_base = ctx.config.resolve_anthropic_api_base();
                let client = match crate::api::AnthropicClient::new(crate::api::client::ClientConfig {
                    api_key: anthropic_key.clone(),
                    api_base: anthropic_base,
                    ..Default::default()
                }) {
                ...
                let all = crate::tools::all_tools();
                let agent_tools: Vec<Box<dyn crate::tools::Tool>> =
                ...
                let mut messages = vec![crate::core::types::Message::user(prompt)];
    ...
    crate::tools::register_agent_runner(runner);
}
