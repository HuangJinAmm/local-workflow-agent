...
use crate::core::codex_oauth::{
    CODEX_API_ENDPOINT, CODEX_MODELS, CODEX_TOKEN_URL, DEFAULT_CODEX_MODEL,
};
use crate::core::oauth_config::{get_codex_tokens, save_codex_tokens, CodexTokens};
...
impl CodexProvider {
    ...
    async fn refresh_token(&self, refresh_token: &str) -> Result<String, ProviderError> {
        let body = json!({
            ...
            "client_id": crate::core::codex_oauth::CODEX_CLIENT_ID,
                                        ...
                                        Some("function_call") => {
                                            ...
                                            if open_blocks.insert(output_index) {
                                                yield Ok(StreamEvent::ContentBlockStart {
                                                    index: output_index,
                                                    content_block: crate::core::types::ContentBlock::ToolUse {
                                                        id: call_id,
                                                        name,
                                                        input: json!({}),
                                                    },
                                ...
                                if let Some(part) = json_val.get("part") {
                                    if matches!(part.get("type").and_then(|value| value.as_str()), Some("output_text") | Some("text")) {
                                        ...
                                        if open_blocks.insert(output_index) {
                                            yield Ok(StreamEvent::ContentBlockStart {
                                                index: output_index,
                                                content_block: crate::core::types::ContentBlock::Text {
                                                    text: String::new(),
                                                },
                                            });
                                        }
