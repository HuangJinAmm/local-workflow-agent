...
async fn run_in_background(
    ...
    tokio::spawn(async move {
                        ...
                        match c.wait().await {
                            ...
                            Ok(status) => {
                                ...
                                global_registry().update_status(
                                    &task_id_clone,
                                    crate::core::tasks::TaskStatus::Failed(
                            ...
                            Err(e) => {
                                global_registry().update_status(
                                    &task_id_clone,
                                    crate::core::tasks::TaskStatus::Failed(e.to_string()),
                                );
                            }
                        }
                    }
        ...
        if result.is_err() {
            global_registry().update_status(
                &task_id_clone,
                crate::core::tasks::TaskStatus::Failed(format!("timed out after {}ms", timeout_ms)),
            );
        }
    });
                    ...
                    match task {
                        Some(t) if matches!(
                            ...
                            crate::core::tasks::TaskStatus::Completed
                                | crate::core::tasks::TaskStatus::Failed(_)
                                | crate::core::tasks::TaskStatus::Cancelled
                        ) => {
                            let exit_info = match &t.status {
                                crate::core::tasks::TaskStatus::Completed => "exit 0".to_string(),
                                crate::core::tasks::TaskStatus::Failed(msg) => {
                                    format!("failed: {}", msg)
                                }
                                crate::core::tasks::TaskStatus::Cancelled => {
                                    "cancelled".to_string()
...
impl Tool for BashTool {
    fn name(&self) -> &str {
        crate::core::constants::TOOL_NAME_BASH
    }
