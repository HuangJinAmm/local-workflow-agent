...
async fn semantic_extraction(html: &str, ctx: &ToolContext) -> Option<String> {
    ...
    let client = match crate::api::AnthropicClient::from_config(&ctx.config) {
        Ok(c) => c,
        Err(e) => {
    ...
    let api_messages = vec![crate::api::ApiMessage {
        role: "user".to_string(),
        content: serde_json::Value::String(user_message),
    }];

    let request = crate::api::CreateMessageRequest::builder("claude-haiku-4-5", 2000)
        .messages(api_messages)
        .system(crate::api::SystemPrompt::Text(system.to_string()))
        .build();
...
impl Tool for WebFetchTool {
    fn name(&self) -> &str {
        crate::core::constants::TOOL_NAME_WEB_FETCH
    }
