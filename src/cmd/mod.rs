...
use crate::core::config::{Config, Settings, Theme};
...
pub struct CommandContext {
    ...
    pub mcp_auth_runner: Option<Arc<dyn Fn(crate::mcp::oauth::McpAuthSession) + Send + Sync>>,
}
...
pub enum CommandResult {
    ...
    ResumeSession(crate::core::history::ConversationSession),
    ...
    ///
    /// `provider` is one of `crate::core::accounts::PROVIDER_ANTHROPIC` or
...
fn resolve_fast_model_id(config: &Config) -> String {
    ...
    let registry = crate::api::ModelRegistry::new();
...
async fn provider_for_config(config: &Config) -> Option<std::sync::Arc<dyn crate::api::LlmProvider>> {
    let anthropic_auth = config.resolve_anthropic_auth_async().await;
    let registry = crate::api::ProviderRegistry::from_config(
        config,
        crate::api::client::ClientConfig {
    ...
    provider_lookup_ids(config.selected_provider_id())
        .into_iter()
        .find_map(|lookup_id| registry.get(&crate::core::ProviderId::new(lookup_id)).cloned())
}
...
fn format_keystroke(keystroke: &crate::core::keybindings::ParsedKeystroke) -> String {
...
fn format_chord(chord: &[crate::core::keybindings::ParsedKeystroke]) -> String {
    chord
        .iter()
...
fn generate_keybindings_template() -> anyhow::Result<String> {
    ...
    for binding in crate::core::keybindings::default_bindings() {
        let chord = format_chord(&binding.chord);
        if crate::core::keybindings::NON_REBINDABLE.contains(&chord.as_str()) {
            continue;
        }
        grouped
...
fn available_output_style_names() -> Vec<String> {
    crate::core::output_styles::all_styles(&Settings::config_dir())
        .into_iter()
...
impl SlashCommand for CostCommand {
    ...
    async fn execute(&self, _args: &str, ctx: &mut CommandContext) -> CommandResult {
        ...
        let pricing = crate::core::cost::ModelPricing::for_model(model);
        ...
        match key {
            ...
            "permission-mode" | "permission_mode" => {
                let mode = match value.trim().to_lowercase().as_str() {
                    "default" => crate::core::config::PermissionMode::Default,
                    "accept-edits" | "accept_edits" => {
                        crate::core::config::PermissionMode::AcceptEdits
                    }
                    "bypass-permissions" | "bypass_permissions" => {
                        crate::core::config::PermissionMode::BypassPermissions
                    }
                    "plan" => crate::core::config::PermissionMode::Plan,
                    _ => {
                        return CommandResult::Error(
...
impl SlashCommand for ResumeCommand {
    ...
    async fn execute(&self, args: &str, _ctx: &mut CommandContext) -> CommandResult {
        if args.is_empty() {
            let sessions = crate::core::history::list_sessions().await;
            if sessions.is_empty() {
                return CommandResult::Message("No previous sessions found.".to_string());
            }
            let last = &sessions[0];
            match crate::core::history::load_session(&last.id).await {
            ...
            match crate::core::history::load_session(args.trim()).await {
                Ok(session) => CommandResult::ResumeSession(session),
                Err(e) => CommandResult::Error(format!(
                    "Failed to load session {}: {}",
                    args.trim(),
...
impl SlashCommand for StatusCommand {
    ...
    async fn execute(&self, _args: &str, ctx: &mut CommandContext) -> CommandResult {
        // Auth status
        let auth_status = match crate::core::oauth::OAuthTokens::load().await {
            Some(tokens) => {
...
impl SlashCommand for GoalCommand {
    ...
    async fn execute(&self, args: &str, ctx: &mut CommandContext) -> CommandResult {
        ...
        match args {
            ...
            "pause" => {
                ...
                match store.get_goal(session_id) {
                    ...
                    Some(g) if g.status == crate::core::GoalStatus::Complete => {
                        return CommandResult::Message("Goal is already complete.".to_string());
                    }
                    Some(g) if g.status == crate::core::GoalStatus::Paused => {
                        return CommandResult::Message(
                ...
                if let Err(e) = store.set_status(session_id, crate::core::GoalStatus::Paused) {
            ...
            "resume" => {
                ...
                match store.get_goal(session_id) {
                    ...
                    Some(g) if g.status == crate::core::GoalStatus::Active => {
                ...
                if let Err(e) = store.set_status(session_id, crate::core::GoalStatus::Active) {
        ...
        match store.set_goal(session_id, objective, token_budget) {
            Err(crate::core::GoalError::ObjectiveTooLong { len, max }) => {
                CommandResult::Error(format!(
            ...
            Ok(goal) => {
                ...
                CommandResult::UserMessage(crate::core::goal_kickoff_message(&goal))
            }
        }
    }
}

fn open_goal_store() -> Option<crate::core::GoalStore> {
    crate::core::GoalStore::open_default()
}
...
impl SlashCommand for UsageCommand {
    ...
    async fn execute(&self, _args: &str, ctx: &mut CommandContext) -> CommandResult {
        ...
        let account_info = match crate::core::oauth::OAuthTokens::load().await {
            Some(tokens) => {
...
impl SlashCommand for PluginCommand {
    ...
    async fn execute(&self, args: &str, ctx: &mut CommandContext) -> CommandResult {
        ...
        async fn get_registry(
            ...
            if let Some(global) = crate::plugins::global_plugin_registry() {
                ...
                crate::plugins::load_plugins(project_dir, &[]).await
            }
        }

        let parsed = crate::plugins::parse_plugin_args(args);
        match parsed {
            crate::plugins::PluginSubCommand::List => {
                let registry = get_registry(&project_dir).await;
                CommandResult::Message(crate::plugins::format_plugin_list(&registry))
            }
            crate::plugins::PluginSubCommand::Enable(ref name) if name.is_empty() => {
                CommandResult::Error(
                    "Usage: /plugin enable <name>\nRun /plugin list to see installed plugins."
                        .to_string(),
                )
            }
            crate::plugins::PluginSubCommand::Enable(name) => {
                let registry = get_registry(&project_dir).await;
                ...
                let mut settings = crate::core::config::Settings::load_sync().unwrap_or_default();
            ...
            crate::plugins::PluginSubCommand::Disable(ref name) if name.is_empty() => {
                CommandResult::Error(
                    "Usage: /plugin disable <name>\nRun /plugin list to see installed plugins."
                        .to_string(),
                )
            }
            crate::plugins::PluginSubCommand::Disable(name) => {
                let registry = get_registry(&project_dir).await;
                if registry.get(&name).is_none() {
                    return CommandResult::Error(format!(
                        "Plugin '{}' not found. Use `/plugin list` to see installed plugins.",
            ...
            crate::plugins::PluginSubCommand::Info(ref name) if name.is_empty() => {
                CommandResult::Error(
                    "Usage: /plugin info <name>\nRun /plugin list to see installed plugins."
                        .to_string(),
                )
            }
            crate::plugins::PluginSubCommand::Info(name) => {
                let registry = get_registry(&project_dir).await;
                CommandResult::Message(crate::plugins::format_plugin_info(&registry, &name))
            }
            crate::plugins::PluginSubCommand::Install(ref path) if path.is_empty() => {
                CommandResult::Error(
                    "Usage: /plugin install <path>\nProvide the path to a local plugin directory."
                        .to_string(),
                )
            }
            crate::plugins::PluginSubCommand::Install(path) => {
                let result = crate::plugins::install_plugin_from_path(
            ...
            crate::plugins::PluginSubCommand::Reload => {
                let old_registry = get_registry(&project_dir).await;
                let (new_registry, diff) =
                    crate::plugins::reload_plugins(&old_registry, &project_dir, &[]).await;
                CommandResult::Message(crate::plugins::format_reload_summary(&new_registry, &diff))
            }
            crate::plugins::PluginSubCommand::Help => {
                CommandResult::Message(
                    "Plugin commands:\n\
                     /plugin              — list all installed plugins\n\
                     /plugin list         — list all installed plugins\n\
...
impl SlashCommand for ReloadPluginsCommand {
    ...
    async fn execute(&self, _args: &str, ctx: &mut CommandContext) -> CommandResult {
        ...
        let old_registry = crate::plugins::load_plugins(&project_dir, &[]).await;
        let (new_registry, diff) =
            crate::plugins::reload_plugins(&old_registry, &project_dir, &[]).await;
...
pub struct PluginSlashCommandAdapter {
    pub def: crate::plugins::PluginCommandDef,
}
...
impl SlashCommand for PluginSlashCommandAdapter {
    ...
    async fn execute(&self, args: &str, _ctx: &mut CommandContext) -> CommandResult {
        ...
        if let Err(reason) = crate::plugins::check_plugin_capability(&self.def) {
            return CommandResult::Error(reason);
        }

        match &self.def.run_action {
            crate::plugins::CommandRunAction::StaticResponse(msg) => {
                CommandResult::Message(msg.clone())
            }
            crate::plugins::CommandRunAction::MarkdownPrompt {
                file_path,
                plugin_root: _,
            } => {
            ...
            crate::plugins::CommandRunAction::ShellCommand {
                command,
                plugin_root,
            } => {
...
impl SlashCommand for DoctorCommand {
    ...
    async fn execute(&self, _args: &str, ctx: &mut CommandContext) -> CommandResult {
        ...
        let client_config = crate::api::client::ClientConfig {
            api_key: anthropic_auth.0,
            api_base: ctx.config.resolve_anthropic_api_base(),
            use_bearer_auth: anthropic_auth.1,
            ..Default::default()
        };
        let provider_registry = crate::api::ProviderRegistry::from_config(&ctx.config, client_config);
        let provider_id = crate::core::ProviderId::new(ctx.config.selected_provider_id());
        match provider_registry.get(&provider_id) {
            Some(provider) => match provider.health_check().await {
                Ok(crate::api::provider_types::ProviderStatus::Healthy) => {
                    lines.push(format!("  ✓ {} is healthy", provider.name()));
                }
                Ok(crate::api::provider_types::ProviderStatus::Degraded { reason }) => {
                    lines.push(format!("  ⚠ {} is degraded: {}", provider.name(), reason));
                }
                Ok(crate::api::provider_types::ProviderStatus::Unavailable { reason }) => {
            ...
            None => {
                let hint = crate::core::config::primary_api_key_env_var_for_provider(
        ...
        let config_dir = crate::core::config::Settings::config_dir();
        ...
        if settings_path.exists() {
            match std::fs::read_to_string(&settings_path)
                .ok()
                .and_then(|s| serde_json::from_str::<crate::core::config::Settings>(&s).ok())
            {
        ...
        if mcp_count == 0 {
            ...
            for srv in ctx.config.mcp_servers.iter().take(12) {
                let status_str = match statuses.get(&srv.name) {
                    Some(crate::mcp::McpServerStatus::Connected { tool_count }) => {
                        format!("  ✓ {} — connected ({} tool{})",
                            srv.name, tool_count, if *tool_count == 1 { "" } else { "s" })
                    }
                    Some(crate::mcp::McpServerStatus::Connecting) => {
                        format!("  ⚠ {} — connecting…", srv.name)
                    }
                    Some(crate::mcp::McpServerStatus::Disconnected { last_error: Some(e) }) => {
                        format!("  ✗ {} — failed: {}", srv.name, e)
                    }
                    Some(crate::mcp::McpServerStatus::Disconnected { last_error: None }) => {
                        format!("  ✗ {} — disconnected", srv.name)
                    }
                    Some(crate::mcp::McpServerStatus::Failed { error, .. }) => {
        ...
        let all_tool_names: Vec<String> = crate::tools::all_tools()
            .iter()
        ...
        let mode_label = match ctx.config.permission_mode {
            crate::core::PermissionMode::BypassPermissions => "bypass-permissions (no confirmation required)",
            crate::core::PermissionMode::AcceptEdits => "accept-edits (file edits auto-approved)",
            crate::core::PermissionMode::Plan => "plan (read-only, no writes)",
            crate::core::PermissionMode::Default => "default (confirm destructive actions)",
        };
        ...
        if ctx.config.permission_mode == crate::core::PermissionMode::Default {
...
impl SlashCommand for LoginCommand {
    ...
    async fn execute(&self, args: &str, _ctx: &mut CommandContext) -> CommandResult {
        ...
        let provider = if use_codex {
            crate::core::accounts::PROVIDER_CODEX
        } else {
            crate::core::accounts::PROVIDER_ANTHROPIC
        };
...
impl SlashCommand for LogoutCommand {
    ...
    async fn execute(&self, args: &str, ctx: &mut CommandContext) -> CommandResult {
        ...
        if use_codex {
            if purge_all {
                let mut registry = crate::core::accounts::AccountRegistry::load();
                let ids: Vec<String> = registry
                    .list(crate::core::accounts::PROVIDER_CODEX)
                    .into_iter()
                    .map(|p| p.id)
                    .collect();
                for id in &ids {
                    let _ = registry.remove(crate::core::accounts::PROVIDER_CODEX, id);
                }
                return CommandResult::Message(format!(
            ...
            if let Err(e) = crate::core::oauth_config::clear_codex_tokens() {
        ...
        if purge_all {
            let mut registry = crate::core::accounts::AccountRegistry::load();
            let ids: Vec<String> = registry
                .list(crate::core::accounts::PROVIDER_ANTHROPIC)
                .into_iter()
                .map(|p| p.id)
                .collect();
            for id in &ids {
                let _ = registry.remove(crate::core::accounts::PROVIDER_ANTHROPIC, id);
            }
            let mut settings = crate::core::config::Settings::load().await.unwrap_or_default();
        ...
        if let Err(e) = crate::core::oauth::OAuthTokens::clear().await {
...
impl SlashCommand for AccountsCommand {
    ...
    async fn execute(&self, _args: &str, _ctx: &mut CommandContext) -> CommandResult {
        let registry = crate::core::accounts::AccountRegistry::load();
        let mut out = String::new();
        for (provider, label) in [
            (crate::core::accounts::PROVIDER_ANTHROPIC, "Anthropic"),
            (crate::core::accounts::PROVIDER_CODEX, "Codex"),
        ] {
...
impl SlashCommand for ReviewCommand {
    ...
    async fn execute(&self, args: &str, ctx: &mut CommandContext) -> CommandResult {
        ...
        // 1. Collect the diff
        // ------------------------------------------------------------------
        let repo_root = crate::core::git_utils::get_repo_root(&ctx.working_dir)
            .unwrap_or_else(|| ctx.working_dir.clone());

        let diff = if base.is_empty() {
            // No base given — use staged changes; fall back to unstaged if empty.
            let staged = crate::core::git_utils::get_staged_diff(&repo_root);
            if staged.is_empty() {
                crate::core::git_utils::get_unstaged_diff(&repo_root)
            } else {
                staged
            }
        } else {
        ...
        let request = crate::api::ProviderRequest {
            model,
            messages: vec![Message::user(review_prompt)],
            system_prompt: Some(crate::api::SystemPrompt::Text(
...
impl McpCommand {
    ...
    async fn handle_auth(server_name: &str, ctx: &CommandContext) -> CommandResult {
        ...
        if !is_http {
            ...
            let token_note = match crate::mcp::oauth::get_mcp_token(server_name) {
                    ...
                    Ok(result) => {
                        ...
                        for msg in &result.messages {
                            let text = match &msg.content {
                                crate::mcp::PromptMessageContent::Text { text } => text.clone(),
                                crate::mcp::PromptMessageContent::Image { .. } => "[image]".to_string(),
        ...
        match sub {
            "set" => {
                let mode = match arg.to_lowercase().as_str() {
                    "default" => crate::core::config::PermissionMode::Default,
                    "accept-edits" | "accept_edits" => crate::core::config::PermissionMode::AcceptEdits,
                    "bypass-permissions" | "bypass_permissions" => crate::core::config::PermissionMode::BypassPermissions,
                    "plan" => crate::core::config::PermissionMode::Plan,
            ...
            "reset" => {
                ...
                new_config.permission_mode = crate::core::config::PermissionMode::Default;
                if let Err(e) = save_settings_mutation(|s| {
                    s.config.allowed_tools.clear();
                    s.config.disallowed_tools.clear();
                    s.config.permission_mode = crate::core::config::PermissionMode::Default;
                }) {
                    return CommandResult::Error(format!("Failed to save: {}", e));
                }
                CommandResult::ConfigChangeMessage(
...
impl SlashCommand for ForkCommand {
    ...
    async fn execute(&self, args: &str, ctx: &mut CommandContext) -> CommandResult {
        ...
        let mut new_session = crate::core::history::ConversationSession::new(
        ...
        match crate::core::history::save_session(&new_session).await {
            Ok(()) => CommandResult::Message(format!(
...
fn export_message_to_markdown(
    ...
    all_messages: &[crate::core::types::Message],
    msg_idx: usize,
) -> String {
    use crate::core::types::{ContentBlock, MessageContent, Role, ToolResultContent};
...
impl SlashCommand for ShareCommand {
    ...
    async fn execute(&self, _args: &str, ctx: &mut CommandContext) -> CommandResult {
        use crate::core::share_export::{share_viewer_url, write_session_html, SessionExportMeta};
...
fn extract_session_urls(messages: &[Message]) -> Vec<String> {
    ...
    for msg in messages {
        let text: String = match &msg.content {
            ...
            crate::core::types::MessageContent::Blocks(blocks) => blocks
                .iter()
...
impl SlashCommand for SkillsCommand {
    ...
    async fn execute(&self, _args: &str, ctx: &mut CommandContext) -> CommandResult {
        ...
        if let Some(registry) = crate::plugins::global_plugin_registry() {
        ...
        let discovered = crate::core::discover_skills(
            &ctx.working_dir,
            &ctx.config.skills,
        );
        ...
        if !discovered.is_empty() {
            let mut disc_list: Vec<(&String, &crate::core::DiscoveredSkill)> =
...
impl SlashCommand for StatsCommand {
    ...
    async fn execute(&self, _args: &str, ctx: &mut CommandContext) -> CommandResult {
        ...
        let user_turns = ctx.messages.iter()
            .filter(|m| m.role == crate::core::types::Role::User)
            .count();
        let assistant_turns = ctx.messages.iter()
            .filter(|m| m.role == crate::core::types::Role::Assistant)
            .count();
...
impl SlashCommand for RenameCommand {
    ...
    async fn execute(&self, args: &str, ctx: &mut CommandContext) -> CommandResult {
        ...
        let request = crate::api::ProviderRequest {
            ...
            system_prompt: Some(crate::api::SystemPrompt::Text(system_prompt.to_string())),
            tools: vec![],
            max_tokens: 64,
            temperature: None,
            top_p: None,
...
fn ui_settings_path() -> std::path::PathBuf {
    crate::core::config::Settings::config_dir().join("ui-settings.json")
}
...
impl SlashCommand for RemoteControlCommand {
    ...
    async fn execute(&self, args: &str, ctx: &mut CommandContext) -> CommandResult {
        let settings = match crate::core::config::Settings::load().await {
            Ok(s) => s,
        ...
        match args.trim() {
            "" | "status" => {
                ...
                let fingerprint = crate::bridge::device_fingerprint();
...
impl SlashCommand for CopyCommand {
    ...
    async fn execute(&self, args: &str, ctx: &mut CommandContext) -> CommandResult {
        ...
        let assistant_msgs: Vec<&crate::core::types::Message> = ctx
            .messages
            .iter()
            .rev()
            .filter(|m| m.role == crate::core::types::Role::Assistant)
...
impl SlashCommand for UpgradeCommand {
    ...
    async fn execute(&self, _args: &str, _ctx: &mut CommandContext) -> CommandResult {
        let current = crate::core::constants::APP_VERSION;
...
impl SlashCommand for RateLimitOptionsCommand {
    ...
    async fn execute(&self, _args: &str, ctx: &mut CommandContext) -> CommandResult {
        ...
        let tier_info = match crate::core::oauth::OAuthTokens::load().await {
            Some(tokens) => {
...
impl SlashCommand for AdvisorCommand {
    ...
    async fn execute(&self, args: &str, _ctx: &mut CommandContext) -> CommandResult {
        let arg = args.trim();
        let settings_dir = crate::core::config::Settings::config_dir();
...
impl SlashCommand for ThinkBackCommand {
    ...
    async fn execute(&self, args: &str, ctx: &mut CommandContext) -> CommandResult {
        ...
        let thinking_blocks: Vec<(usize, String)> = ctx
            ...
            .iter()
            .enumerate()
            .filter(|(_, m)| m.role == crate::core::types::Role::Assistant)
            .filter_map(|(idx, m)| {
                ...
                let thinking: String = blocks
                    .iter()
                    .filter_map(|b| {
                        if let crate::core::types::ContentBlock::Thinking { thinking, .. } = b {
...
impl SlashCommand for SearchCommand {
    ...
    async fn execute(&self, args: &str, _ctx: &mut CommandContext) -> CommandResult {
        ...
        let db_path = crate::core::config::Settings::config_dir().join("sessions.db");

        let store = match crate::core::SqliteSessionStore::open(&db_path) {
            Ok(s) => s,
            Err(e) => {
                return CommandResult::Error(format!(
...
mod teleport_bundle {
    use crate::core::permissions::{PermissionAction, SerializedPermissionRule};
        ...
        match sub {
            "export" => {
                ...
                let files: Vec<String> = {
                    use crate::core::types::{ContentBlock, MessageContent};
                ...
                let mut redacted_env_vars: std::collections::HashSet<String> = ctx
                    ...
                    .flat_map(|provider_id| {
                        crate::core::config::api_key_env_vars_for_provider(provider_id)
                            .iter()
                            .copied()
                    })
                    .map(str::to_string)
                ...
                redacted_env_vars.extend(
                    crate::core::config::api_key_env_vars_for_provider(ctx.config.selected_provider_id())
                        .iter()
                        .copied()
                        .map(str::to_string),
                );
    ...
    async fn execute(&self, _args: &str, ctx: &mut CommandContext) -> CommandResult {
        ...
        let (conv_chars, tool_chars): (usize, usize) = ctx.messages.iter().fold(
            ...
            |(conv, tool), msg| {
                ...
                if msg.role == crate::core::types::Role::User && text.starts_with('[') {
                    (conv, tool + text.len())
                } else {
                    (conv + text.len(), tool)
                }
...
impl SlashCommand for InsightsCommand {
    ...
    async fn execute(&self, _args: &str, ctx: &mut CommandContext) -> CommandResult {
        ...
        let assistant_turns: usize = messages.iter()
            .filter(|m| matches!(m.role, crate::core::types::Role::Assistant))
            .count();
        ...
        for msg in messages {
            for block in msg.get_tool_use_blocks() {
                if let crate::core::types::ContentBlock::ToolUse { name, .. } = block {
...
impl SlashCommand for RevertCommand {
    ...
    async fn execute(&self, args: &str, ctx: &mut CommandContext) -> CommandResult {
        let snap = match crate::core::snapshot::get_or_create(&ctx.working_dir) {
            Some(s) => s,
        ...
        let checkpoints: Vec<&crate::core::types::Message> = ctx.messages.iter()
            .filter(|m| {
                m.role == crate::core::types::Role::Assistant
        ...
        let patches: Vec<crate::core::snapshot::Patch> = ctx.messages.iter()
        ...
        let project_root = crate::core::git_utils::get_repo_root(&ctx.working_dir)
            .unwrap_or_else(|| ctx.working_dir.clone());
        let path = crate::core::session_storage::transcript_path(&project_root, &ctx.session_id);
        if path.exists() {
            if let Err(e) = crate::core::session_storage::truncate_after(&path, &target_uuid).await {
...
impl SlashCommand for CheckpointsCommand {
    ...
    async fn execute(&self, _args: &str, ctx: &mut CommandContext) -> CommandResult {
        let checkpoints: Vec<(usize, &crate::core::types::Message)> = ctx.messages.iter()
            .enumerate()
            .filter(|(_, m)| {
                m.role == crate::core::types::Role::Assistant
                    && m.snapshot_patch.is_some()
...
impl SlashCommand for SnapshotDiffCommand {
    ...
    async fn execute(&self, args: &str, ctx: &mut CommandContext) -> CommandResult {
        ...
        let hash = if !args.is_empty() && args.chars().all(|c| c.is_ascii_hexdigit()) && args.len() >= 8 {
            ...
            let checkpoints: Vec<&crate::core::snapshot::Patch> = ctx.messages.iter()
                .filter_map(|m| {
                    if m.role == crate::core::types::Role::Assistant {
...
impl SlashCommand for AgentCommand {
    ...
    async fn execute(&self, args: &str, ctx: &mut CommandContext) -> CommandResult {
        ...
        let mut all_agents: HashMap<String, crate::core::AgentDefinition> =
            crate::core::default_agents();
...
impl SlashCommand for ManagedAgentsCommand {
    ...
    async fn execute(&self, args: &str, ctx: &mut CommandContext) -> CommandResult {
        use crate::core::{BudgetSplitPolicy, ManagedAgentConfig, builtin_managed_agent_presets};
...
struct TemplateCommand {
    name: String,
    template: crate::core::CommandTemplate,
}
...
/// `settings.commands`.
pub fn commands_from_settings(settings: &crate::core::Settings) -> Vec<Box<dyn SlashCommand>> {
...
pub fn commands_from_discovered_skills(
    cwd: &std::path::Path,
    skills_config: &crate::core::SkillsConfig,
) -> Vec<Box<dyn SlashCommand>> {
    let discovered = crate::core::discover_skills(cwd, skills_config);
...
pub async fn execute_command(
    ...
    {
        let discovered = crate::core::discover_skills(&ctx.working_dir, &ctx.config.skills);
    ...
    let project_dir = ctx.working_dir.clone();
    let registry = crate::plugins::load_plugins(&project_dir, &[]).await;
    ...
    async fn test_login_command_starts_oauth_flow() {
        ...
        match result {
            CommandResult::StartLoginForProvider {
                ...
                assert_eq!(provider, crate::core::accounts::PROVIDER_ANTHROPIC);
    ...
    async fn test_login_command_codex_flag() {
        ...
        match result {
            CommandResult::StartLoginForProvider {
                ...
                assert_eq!(provider, crate::core::accounts::PROVIDER_CODEX);
