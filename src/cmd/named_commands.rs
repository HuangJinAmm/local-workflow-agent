    ...
    fn execute_named(&self, args: &[&str], ctx: &CommandContext) -> CommandResult {
        match args.first().copied().unwrap_or("list") {
            "list" => {
                ...
                let defs = crate::tui::agents_view::load_agent_definitions(&ctx.working_dir);
...
impl NamedCommand for AddDirCommand {
    ...
    fn execute_named(&self, args: &[&str], _ctx: &CommandContext) -> CommandResult {
        ...
        let mut settings = match crate::core::config::Settings::load_sync() {
            Ok(s) => s,
            Err(e) => {
    ...
    fn execute_named(&self, args: &[&str], ctx: &CommandContext) -> CommandResult {
        match args.first().copied().unwrap_or("") {
            "" | "create" => {
                ...
                let result = tokio::task::block_in_place(|| {
                    tokio::runtime::Handle::current().block_on(async move {
                        crate::core::history::branch_session(
                            &session_id,
                            msg_count,
                            title_opt.as_deref(),
                        )
            ...
            "list" => {
                ...
                let sessions = tokio::task::block_in_place(|| {
                    tokio::runtime::Handle::current()
                        .block_on(crate::core::history::list_sessions())
                });
            ...
            "switch" => {
                ...
                let result = tokio::task::block_in_place(|| {
                    tokio::runtime::Handle::current()
                        .block_on(crate::core::history::load_session(&id))
                });
    ...
    fn execute_named(&self, args: &[&str], ctx: &CommandContext) -> CommandResult {
        ...
        match args.first().copied().unwrap_or("list") {
            "list" => {
                let result = tokio::task::block_in_place(|| {
                    tokio::runtime::Handle::current()
                        .block_on(crate::core::history::load_session(&session_id))
                });
                match result {
                    Ok(session) => {
            ...
            "add" => {
                ...
                let result = tokio::task::block_in_place(|| {
                    tokio::runtime::Handle::current()
                        .block_on(crate::core::history::tag_session(&session_id, &tag))
                });
            ...
            "remove" => {
                ...
                let result = tokio::task::block_in_place(|| {
                    tokio::runtime::Handle::current()
                        .block_on(crate::core::history::untag_session(&session_id, &tag))
                });
                    ...
                    Ok(session) => {
                        ...
                        if session.tags.iter().any(|t| t == &tag) {
                            ...
                            let remove_result = tokio::task::block_in_place(|| {
                                tokio::runtime::Handle::current()
                                    .block_on(crate::core::history::untag_session(&session_id, &tag_clone))
                            });
                            match remove_result {
                                Ok(()) => CommandResult::Message(format!("Removed tag: #{tag}")),
                                Err(e) => CommandResult::Error(format!("Could not remove tag: {e}")),
                            ...
                            let add_result = tokio::task::block_in_place(|| {
                                tokio::runtime::Handle::current()
                                    .block_on(crate::core::history::tag_session(&session_id, &tag_clone))
                            });
                            match add_result {
                                Ok(()) => CommandResult::Message(format!("Added tag: #{tag}")),
...
impl NamedCommand for IdeCommand {
    ...
    fn execute_named(&self, _args: &[&str], _ctx: &CommandContext) -> CommandResult {
        ...
        let env_detection = crate::core::detect_ide();
...
impl NamedCommand for InstallGithubAppCommand {
    ...
    fn execute_named(&self, _args: &[&str], ctx: &CommandContext) -> CommandResult {
        ...
        let provider_secret_step = crate::core::config::primary_api_key_env_var_for_provider(provider_id)
            .map(|provider_secret| {
                format!(
...
impl NamedCommand for RemoteSetupCommand {
    ...
    fn execute_named(&self, _args: &[&str], ctx: &CommandContext) -> CommandResult {
        ...
        let credential_hint = crate::core::config::api_key_env_vars_for_provider(provider_id);
